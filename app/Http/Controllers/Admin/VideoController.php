...
class VideoController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            {
                $nav = 'Menampilkan Video 0 - 0 Dari 0';
            }else{
                $nav = 'Menampilkan Video '. $data->toArray()['from'] .' - '.$data->toArray()['to'] .' Dari '.$data->toArray()['total'];
            }
            ...
            return response()->json([
                ...
                'prev_page' => $prev,
                'navigasi' => $nav,
                'html' => view('admin.galeri.video.incl.data', compact('data'))->render(),
            ]);
        }
    ...
    public function edit($id){
        $video = Video::find($id);
        return view('admin.galeri.video.edit', compact('video'));
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()){
            ...
            try{
                $data = Video::find($request->video_id);
                if(!empty($request->cover_url))
                {
